 ...
 */
	protected function _setupAuth() {
		...
		$this->Auth->allow('add', 'reset', 'verify', 'logout', 'view', 'reset_password', 'login','crmlogin', 'resend_verification');
		...
		if ($this->request->action == 'register') {
			$this->Components->disable('Auth');
                ...
                //echo '<pre>';
                //echo $user=$this->Auth->user('is_admin');
                /*if($user!=1)
                {
                    //die('hello');
                }*/
		...
		$this->Auth->logoutRedirect = '/';
                $this->Auth->logoutAction = array('plugin' => Inflector::underscore($this->plugin), 'controller' => 'users', 'action' => 'crmlogin');
		$this->Auth->loginAction = array('admin' => false, 'plugin' => Inflector::underscore($this->plugin), 'controller' => 'users', 'action' => 'crmlogin');
	}
...
//                                    die('hello');
					$this->Auth->loginRedirect = 'crm_companies/add';
				}
				...
				if (method_exists($this->Auth, 'redirectUrl')) {
                                    $data[$this->modelClass]['return_to']='users/index';
 ...
 */
    ...
    /**
     * crm login action
     *All in one login (Any type of user can login from here , user will be redirected to their dashboard according to their role)
     * @return void
     */
       ...
       // $this->layout = "login";
        ...
        if ($this->request->is('post')) {
            if ($this->Auth->login()) {
                ...
                if ($this->here == $this->Auth->loginRedirect) {
//                    $this->Auth->loginRedirect = '/admins/dashboard';
                }
//                $this->Auth->loginRedirect = '/admins/dashboard';
                $this->Session->setFlash(sprintf(__d('users', '%s you have successfully logged in'), $this->Auth->user($this->{$this->modelClass}->displayField)));
                if (!empty($this->request->data)) {
                ...
                if ($_SESSION['Auth']['User']['role'] == "admin") {
                    $this->Auth->loginRedirect = '/crm_companies';
                }
                 if ($_SESSION['Auth']['User']['role'] == "company") {
                     $companyID=  base64_encode($_SESSION['Auth']['User']['crm_company_id']);
                    $this->Auth->loginRedirect = "/employees/index/$companyID";
                }
                 if ($_SESSION['Auth']['User']['role'] == "employee") {
                     $userID=$this->Session->read('Auth.User.id');
                    $this->Auth->loginRedirect = "/employees/view/$userID";
                }
//                 if ($_SESSION['Auth']['User']['role'] == "company") {
//                    $this->Auth->loginRedirect = '/employees';
//                }
                // Checking for 2.3 but keeping a fallback for older versions
                if (method_exists($this->Auth, 'redirectUrl')) {
                    $this->redirect($this->Auth->redirectUrl($data[$this->modelClass]['return_to']));
       ...
       // $this->layout = "login";
        ...
        if ($this->request->is('post')) {
            if ($this->Auth->login()) {
                ...
                if ($_SESSION['Auth']['User']['role'] == "company") {
                    $this->Auth->loginRedirect = '/employeeswe';
                }
                // Checking for 2.3 but keeping a fallback for older versions
                if (method_exists($this->Auth, 'redirectUrl')) {
                    $this->redirect($this->Auth->redirectUrl($data[$this->modelClass]['return_to']));
